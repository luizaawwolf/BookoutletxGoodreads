# -*- coding: utf-8 -*-
"""
Created on Mon Jul  6 13:31:12 2020
...
url = "https://www.secondsale.com/search?controller=search&s=chain+of+gold&bookCondition=buy&btnBuySubmit="
#url = 'https://bookoutlet.com/Store/Search?qf=All&q=' + title_url
response = requests.get(url)
soup = BeautifulSoup(response.text, 'html.parser')
search_results = soup.findAll('article','product-miniature')
for result in search_results:
    author = result.find('div','author_div').find('a').find_all(text=True)[0]
    author = author.replace('\n','')
    title = result.find('a', 'thumbnail')['title']
    price = result.find('div','product-price-and-shipping').find('span','price').find_all(text=True)
    if len(price) > 1:
        price = price[1]
    else:
        continue
